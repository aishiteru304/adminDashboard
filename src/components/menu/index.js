        ...
        <div className={styles.wrapMenu} id='Menu'>
            ...
            <div className={`borderBottom ${styles.borderMaterial}`}></div>
            {
                menuItem.map((item, index) => (
                    <Link to={`/${item.title}`} className={styles.menuItem} key={index} style={{ backgroundColor: selected[index] ? 'var(--menu-item-color)' : '', color: 'var(--menu-color)' }} onClick={() => handleSelect(index)}>
                        {item.icon}
